...
bool sound_Prompt(void) {
	int i;
	char prompt[29] = {0xFD, 0x00, 0x1A, 0x01, 0x03, 0x73, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x6E, 0x00, 0x64, 0x00, 0x32, 0x00, 0x32, 0x00, 0x35, 0x00,
                     0xC6, 0x51, 0x07, 0x59, 0x31, 0x5C, 0xEA, 0x7E}; //准备就绪
...
bool sound1_Prompt(void) {
	int i;
	char prompt[33] = {0xFD, 0x00, 0x1E, 0x01, 0x03, 0x73, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x6E, 0x00, 0x64, 0x00, 0x33, 0x00, 0x31, 0x00, 0x32, 0x00,
		                 0x4B, 0x62,0x3A, 0x67, 0x61, 0x53, 0x2A, 0x67, 0xD2, 0x63, 0x65, 0x51}; //手机卡未插入
...
bool sound2_Prompt(void) {
	int i;
	char prompt[39] = {0xFD, 0x00, 0x24, 0x01, 0x03, 0x73, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x6E, 0x00, 0x64, 0x00, 0x33, 0x00, 0x30, 0x00, 0x38, 0x00, 
		      0x2A, 0x67, 0xFD, 0x80, 0xA5, 0x63, 0x65, 0x51, 0x0D, 0x67, 0xA1, 0x52, 0x68, 0x56, 0x73, 0x5E, 0xF0, 0x53}; //未能接入服务器平台
...
bool sound3_Prompt(void) {
	int i;
	char prompt[39] = {0xFD, 0x00, 0x24, 0x01, 0x03, 0x73, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x6E, 0x00, 0x64, 0x00, 0x32, 0x00, 0x31, 0x00, 0x38, 0x00,
		       0x4B, 0x62, 0x3A, 0x67, 0x21, 0x6A, 0x57, 0x57, 0x1D, 0x52, 0xCB, 0x59, 0x16, 0x53, 0x10, 0x62, 0x9F, 0x52}; //手机模块初始化成功
  SoundControlSetChannel(SOUND_CONTROL_CHANNEL_XFS, 1);
	for (i = 0; i < 39; i++) {
		USART_SendData(USART3, prompt[i]);
		while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);
...
bool sound4_Prompt(void) {
	int i;
	char prompt[39] = {0xFD, 0x00, 0x24, 0x01, 0x03, 0x73, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x6E, 0x00, 0x64, 0x00, 0x33, 0x00, 0x30, 0x00, 0x34, 0x00,
		0x67, 0xF9, 0xFD, 0x80, 0x0E, 0x4E, 0x4B, 0x62, 0x3A, 0x67, 0x21, 0x6A, 0x57, 0x57, 0x1A, 0x90, 0xAF, 0x8B}; //不能与手机模块通讯
  SoundControlSetChannel(SOUND_CONTROL_CHANNEL_XFS, 1);
	for (i = 0; i < 39; i++) {
		USART_SendData(USART3, prompt[i]);
		while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);
